...
export function p4Failures(model, plan) {
  ...
  const expectedLayout = model.documentGraph.nodes.map((node) => {
    ...
    const local = localTransform(source);
    const world = node.parentId ? multiply(worldById.get(node.parentId), local) : structuredClone(local);
  ...
  const expectedBindings = model.bindingGraph.records.map((record) => {
    ...
    return bindingOwner(record, 'semantic', record.source.nodeId);
  });
  ...
  const G6 = plan.modelContentSeal !== model.contentSeal || plan.rootId !== model.documentGraph.rootId || canonicalJson(plan.layout?.nodes) !== canonicalJson(expectedLayout) || canonicalJson(actualBindings) !== canonicalJson(expectedBindings) || !ownersResolve;
  const G7 = canonicalJson(plan.render?.nodes) !== canonicalJson(expectedRenderNodes) || canonicalJson(plan.render?.stackingContexts) !== canonicalJson(expectedStacking) || canonicalJson(plan.render?.maskGroups) !== canonicalJson(expectedMasks) || canonicalJson(plan.sourceMap?.fragments) !== canonicalJson(expectedFragmentMap);
  return { G6, G7 };
}
