 ...
 *
 * Usage:
 *   node scripts/tokens/build-tokens.mjs [path-to-json]
 *
 * Default JSON path:
...
const DEFAULT_JSON = CONFIG.sourcePath;
...
const HEADER = `/* Auto-generated by build-tokens.mjs from Figma Variables Export
   Source: onemo-ssot-global/11-design-system/artifacts/figma-variables-2026-03-06.json
   DO NOT EDIT — regenerate with: node scripts/tokens/build-tokens.mjs */\n\n`;
...
// ─── Fluid Token Resolution ──────────────────────────────────────────────────

/**
 * Resolve a token node to a literal px number, following reference chains.
 * Uses `preferredMode` to pick the right mode when the target collection has
 * multiple modes (e.g. Desktop vs Mobile on fluid primitive collections).
 */
...
/**
 * Resolve a token node to { desktopPx, mobilePx } by following its reference
 * chain into a collection that has Desktop and Mobile modes.
 *
 * Handles the full chain:
 *   3.2_Semantic_Spacing (Value) → 2.4_Alias_Spacing (Desktop/Mobile)
 *   → 1.3_Primitive_Fluid_Spacing (Desktop/Mobile) → literal px
 *
 * Returns null if the token does not resolve through a fluid Desktop/Mobile split.
 */
...
/**
 * Generate a CSS clamp() value using the Utopia fluid formula.
 *   slope     = (maxPx − minPx) / (maxWidth − minWidth)
 *   intercept = minPx − slope × minWidth
 *   output    = clamp(minRem, interceptRem + slope×100vi, maxRem)
 *
 * minPx = Mobile value (minSize), maxPx = Desktop value (maxSize).
 * Returns a static rem value when min === max, "0px" when both are zero.
 */
...
function generateAliasesCSS(collections) {
  if (!CONFIG.emitAlias) {
    return {
      css: `${HEADER}/* Alias layer suppressed — internal use only (CONFIG.emitAlias = false) */\n`,
      count: 0,
    };
  }
  let typography;
...
function generateSemanticCSS(collections) {
  ...
  // Font Families (4)
  // When alias layer is suppressed, resolve the full alias → primitive chain to get
  // the actual primitive font var (e.g. --primitive-font-chillax, not --alias-font-primary).
  ...
  // Spacing — fluid clamp() via 3.2 → 2.4 → 1.3 chain (Utopia formula).
  // Top-level tokens are flat; inset/stack/inline are nested groups → flatten with hyphen.
  lines.push('\n  /* ═══ Spacing ═══ */\n');
  ...
  function emitSpacingToken(cssKey, node) {
    if (!node || node.$value === undefined) return; // skip non-leaf
    const fluid = resolveFluidValues(node, collections);
    let cssValue;
    if (fluid) {
      cssValue = generateClamp(
        fluid.mobilePx, fluid.desktopPx,
        CONFIG.viewportBounds.min, CONFIG.viewportBounds.max,
      );
    } else {
      const resolvedValue = resolveReference(node.$value, node.$collectionName || '_Primitives', collections);
      cssValue = typeof resolvedValue === 'number'
  ...
  for (const [key, node] of Object.entries(spacingMode)) {
    ...
    if (node && node.$value !== undefined) {
      // Leaf token — flat spacing var (e.g. --spacing-xs)
      emitSpacingToken(key, node);
    } else if (node && typeof node === 'object') {
      // Group node (e.g. spacing-inset, spacing-stack, spacing-inline)
      // Flatten: --spacing-inset-xs, --spacing-inset-m, etc.
      for (const [subKey, subNode] of Object.entries(node)) {
        if (subKey.startsWith('$')) continue;
        emitSpacingToken(`${key}-${subKey}`, subNode);
      }
    }
  }
...
function generateSemanticInlineCSS(collections) {
  ...
  // Section 2: Dark mode overrides — class selector
  ...
  // Section 2b: Dark mode overrides — media query (system preference)
  lines.push(`/* ═══ Dark mode overrides (system preference) ═══ */\n${CONFIG.darkMode.media} {\n  :root:not([data-theme="light"]) {\n`);
...
function generateTokenReference(collections) {
  const lines = [
    ...
    '> Auto-generated by `node scripts/tokens/build-tokens.mjs`.\n\n',
...
function printHelp() {
  console.log('Usage: node scripts/tokens/build-tokens.mjs [--input PATH] [--output-dir PATH] [--validate] [--stats] [--diff] [--help]');
  console.log('');
  console.log('Flags:');
...
function validateGeneratedFiles(generatedFiles) {
  ...
  // 7. Token count sanity
  {
    ...
    for (const fileName of OUTPUT_FILES) {
      ...
      // aliases.css may be intentionally empty when CONFIG.emitAlias = false
      const intentionallyEmpty = fileName === 'aliases.css' && !CONFIG.emitAlias;
      if (count === 0 && !intentionallyEmpty) pass = false;
    }
    checks.push({
