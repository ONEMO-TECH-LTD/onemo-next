...
editor.once('load', () => {
    ...
    const applyOne = (resourceId: string, hidden: boolean) => {
        ...
        try {
            editor.call('r3f:bridge:setHidden', resourceId, hidden);
        } catch (err) {
            // The R3F bridge is mounted after the legacy viewport bootstrap, so
            // keep the hidden canvas sync as a fallback when it is unavailable.
        }
