...
import { GltfExporter, UsdzExporter } from '@/common/playcanvas-compat';
...
editor.once('load', () => {
    ...
    editor.once('entities:load', () => {
        ...
        menuData.push({
            ...
            shortcut: 'F2',
            onIsVisible: hasWriteAccess,
            onIsEnabled: () => items.length === 1,
            onSelect: () => {
                const item = editor.call('entities:panel:get', items[0].get('resource_id'));
                item?.rename();
            }
        });
