...
function SidebarProjectRenderer( editor ) {
	...
	function resolveOutputColorSpace( value ) {

		if ( value === 'srgb' || value === 'srgb-linear' ) return THREE.SRGBColorSpace;
		if ( value === 'linear-srgb' ) return THREE.LinearSRGBColorSpace;
		return value;

	}

	editor.applyRendererSettings = function ( settings ) {

		if ( currentRenderer === null ) return;

		if ( settings.shadowsEnabled !== undefined ) shadowsBoolean.setValue( settings.shadowsEnabled );
		if ( settings.shadows !== undefined ) shadowsBoolean.setValue( settings.shadows );
		if ( settings.shadowType !== undefined ) shadowTypeSelect.setValue( settings.shadowType );
		if ( settings.toneMapping !== undefined ) toneMappingSelect.setValue( settings.toneMapping );

		const exposure = settings.toneMappingExposure ?? settings.exposure;
		if ( exposure !== undefined ) toneMappingExposure.setValue( exposure );
		if ( settings.outputColorSpace !== undefined ) currentRenderer.outputColorSpace = resolveOutputColorSpace( settings.outputColorSpace );
