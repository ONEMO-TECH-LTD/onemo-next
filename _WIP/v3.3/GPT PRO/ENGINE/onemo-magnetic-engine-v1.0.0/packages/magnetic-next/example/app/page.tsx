...
export default function MagneticStudioPage(){
  ...
  return <main><h1>ONEMO Effects Studio — magnetic sizing</h1><p>Replace the example outline with the editor&apos;s validated millimetre contour.</p>
    ...
    {state.options.map(option=><section key={option.band}><h2>{option.label}</h2>{state.result?.offers.find(o=>o.band===option.band)?.solution&&<ShapeSolutionOverlay solution={state.result.offers.find(o=>o.band===option.band)!.solution!} coordinateQuantumMm={profile.numeric.coordinateQuantumMm}/>}</section>)}
  </main>;
}
