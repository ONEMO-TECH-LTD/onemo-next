...
- [ ] **EC-11b · Raw families and user-facing offerings.** The canonical answer always contains every lawful family in canonical order, and the admin surface steps through that order without omission. A user-facing offering set is a DERIVED VIEW over those immutable families: it may mark, order or index them, and may never delete, merge or hide one. Until the minimum separation between offered sizes and the deterministic thinning rule are ruled and supplied as guarded product inputs, the answer reports `offerings.status = separation-policy-unresolved`, the raw family list complete, and no ladder. The engine may not guess a separation value, fall back to first-fit, or filter in the interface. An absent four-corner optimum remains absent: a ladder degrades only through existing intermediate and floor families and never manufactures one.
