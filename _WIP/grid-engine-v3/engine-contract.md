...
- [ ] **EC-08 · Centring and balance evidence.** Every family follows EC-07's parity-derived registration and reports its relationship to each tested shape centre, every side reach, the side-reach spread and the limiting contacts. These are comparison evidence for the applied real-cutout corpus, not an automatic argmin gate: a variant does not fail merely because another variant has a smaller spread. The engine invents no displacement threshold, tolerance, combined score or hidden default. Contested centre constructions remain visible test options.
...
- No product winner, ranking framework or default centre method.
- No rotation.
