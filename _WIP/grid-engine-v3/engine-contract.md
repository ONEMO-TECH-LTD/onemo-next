...
- [ ] **EC-08 · Centring and balance yardstick.** Within each family and band, the centred candidate is the variant that exactly minimizes `max(sideReach) - min(sideReach)` across all sides simultaneously. It is tight when the supported assembly reaches its limiting outline contacts and no uniformly smaller lawful size preserves the same layout, coverage and balance. The engine reports and draws every side reach, the spread and the limiting contacts; it invents no numeric acceptability threshold, tolerance or combined score. Absolute acceptability of the resulting spread is confirmed on the applied real-cutout corpus. Centre constructions remain visible test options, but they compete on this same measure and none becomes a hidden default. Intermediate variants remain reported and do not fail merely because another variant wins the centring comparison.

- [ ] **EC-09 · Flap-limit test.** For every side and material extremity, the engine measures unsupported reach to the nearest supporting magnet disc. The flap limit is an explicit test switch with exactly two positions: 12mm and 24mm. Coverage passes only when every side is within the selected limit. Reach beyond the selected limit fails that test unless EC-10 applies. No intermediate threshold or hidden default is permitted.
