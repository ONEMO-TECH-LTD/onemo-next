...
- [ ] **EC-06 · Complete material support, and the grid region inside the material.** The shape must **encapsulate the layout's grid region** — the magnet extent grown by the padding on every side. *(The region is the union of the adjacent pairs' padded boxes — confirmed by Dan; see EC-09.)* Containment is closed: the region's boundary may touch the outline. This is strictly stronger than per-magnet containment and is the engine's central computation — *Dan, 2026-08-11: "so the engine computing is essentially whether the bounding box fits inside the shape in set variants of layouts based on the grid".* Measured: it preserves the square canon exactly (72 / 120 / 168) and raises the circle by 10mm at every band (92→102, 160→170, 228→238) because a box corner must sit on fabric.
...
- [ ] **EC-11a · The corpus bar.** Every shape in the saved cut-out library yields **at least two to three good sizes at FOUR points**. *Dan, 2026-08-11: "each shape in the cutout library can have at leas 2-3 best optimal sizes with 4 points".* This is a pass condition on the delivery, not a per-family check: an engine that satisfies EC-01 through EC-12 and misses this bar has failed. Current measurement does not meet it — on a bounding-box centre BAT-WOMAN reaches one band and DUCK finds nothing — so the centre construction is load-bearing for it.

- [ ] **EC-11b · The twin fix and the user-facing ladder.** A **twin fix** is two magnets sandwiching the garment, one above and one below, holding by magnetic force. Any effect whose **longest side is under 192mm** may be a twin fix — 192 being band 4's 168mm span plus the 24mm stick-out — and there is **no shape condition**. The bar is on the fixing, not on shapes globally: three or more magnets are bounded only by the grid count. Larger sizes are admissible where the garment has a magnetic grid built in. Separately, the sizes offered to a user are **cherry-picked for even separation** rather than listed raw: the lawful ladder clusters (2mm apart is common) and a minimum separation is applied as a declared switch, never an invented number.
