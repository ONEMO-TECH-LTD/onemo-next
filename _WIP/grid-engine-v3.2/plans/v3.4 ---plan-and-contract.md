...
**A — squares first, then rectangles.** Feed the **shape**, not a size. Phase 1 is one square (aspect 1:1, no millimetres). The engine must return the whole band ladder. Rectangles at their aspects come later, same form. Fail = engine wrong. Stop. Do not tune on bats.
...
**Phase 1 — one square, the whole ladder.** New exam through the real door. No engine change yet.

**Input:** a square. Aspect 1:1. No size. Do not pre-draw a millimetre outline and do not have the engine invent the shape from the grid.

**Must return** (per-band selections, all at once):
...
Those sizes are grid arithmetic, not a stored table: span = (n−1)·48 + 24 (LS §5.1). Leftover ~0 at every band. A right count at the wrong size, or the box centre in the wrong place, is a fail. Rectangles later, same shape-in / ladder-out form — not this phase.
...
**Done when:** Phase 1 square ladder green. Observe the seven cutouts before and after the Phase 2 delete (what is refused — not a stopwatch).
