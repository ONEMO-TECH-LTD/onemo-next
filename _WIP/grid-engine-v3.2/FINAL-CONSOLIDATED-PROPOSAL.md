...
> it. The causes trace to this document — its Replace list authorised deleting `fullest-population`,
> and its T6 wording *"enumerate permitted frame hypotheses within those capacities"* was implemented
> as an exclusive filter, which canon forbids in four places.
>
> **Canon is two documents only** (Dan, 2026-08-17): `canon/logic-spec-optimum.md` and
> `canon/ONEMO Magnetic Grid Compute System — Product Base and Logic Architecture.md`.
> This proposal was never one of them — it is an execution plan, and it does not appear in
> logic-spec §5's authority order at all.
>
> **Kept as provenance, and it remains useful for exactly two things:**
> 1. **The v3.3 donor rule** (below, "Governing boundary"), which still governs how parts cross from
>    the v3.3.1 build: a mechanism may cross only when a named task requires it, the v3.2 substrate
>    lacks it, and it passes that task's own gate.
> 2. **The record of what was authorised**, which is how the T1 deletions and the T6 frame reading
>    were traced to their source rather than guessed at.

**Owner:** s62-grid-meta-qa

**Status:** SUPERSEDED. Historical text follows unchanged below — QA-corrected proposal, amended after a full-folder necessity/deslop sweep. No product code is authorised until T0 compiles the settled rules from the designated briefs and Dan gives the build go.
