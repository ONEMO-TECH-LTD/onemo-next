...
A candidate's count, centre, parity class and binding element can change with scale. The engine does not enumerate those changes globally. The existing band walk steps each B1-B4 band upward through the frozen Centre path as a cheap observer and records every new state: count, selected centre/layout, parity/phase, lattice indices and belt membership. For each new state and its discovery bracket, compute enumerates the finite belt-disc × supplied-segment contact equations (endpoint and interior projection, each a quadratic in scale) and solves their exact rational/quadratic roots inside the bracket and band. No sampled size, report decimal or heuristic nearest-edge cutoff decides Wrap. Coverage of discovery is measured, not proved: a denser-step comparison on the §9.4 shapes is QA evidence, and a missed count is a fixture before it is a mechanism.
...
function solveBands(input: SolveBandsInput): AllBandsResult {
  ...
  for (const band of BANDS) {
    // 1. cheap discovery: the existing walk through the frozen numeric Centre path records every new state
    for (const { numeric, bracket } of discoverStates(prepared, band, policy)) {
      // 2. construct: finite local contact roots of that state inside its bracket
      for (const root of contactRoots(exactSelectedState(prepared, numeric, bracket[0]), bracket, geometry)) {
        // 3. validate: one full-law judgement at the exact root; numeric Centre re-run once to confirm the same state
        const state = exactSelectedState(prepared, reselect(prepared, root.scale, policy), root.scale)
        if (!sameState(state, numeric)) continue        // changed state → discard, discovery continues; no recursion
        verdicts.push(evaluateCandidateLaws(rootedFrom(state, judgeState(state, geometry), root), policy))
      }
    }
  }
