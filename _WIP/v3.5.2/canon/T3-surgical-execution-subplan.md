...
| W2 | Delete the exact layer and dead float Wrap helpers + guard asserts absence | §3 table | fixture 4; all remaining tests green; Centre fixture 1 unchanged |
...
`src/lib/magnetic-grid/{spec,compute,logic,engine}.ts`, `src/lib/magnetic-grid/compute/{seat,centre-evidence,contact-root,identity}.ts`, `src/lib/magnetic-grid/__tests__/*`, and from S5: `law.worker.ts`, `LawPanel.tsx`, `magnetic-grid-bridge.ts`. Nothing else; no new files.
