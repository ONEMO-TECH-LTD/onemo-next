...
3. **Scaling:** across B1-B4, counts are strictly increasing; each rung is the earliest accepted exact scale for that count; a count owned by a lower band never repeats; a lawful lower count survives a higher count's refusal; the squircle publishes 8 in B4 (its B3 seat-only appearance is not ownership).
...
10. **Discovery coverage (QA evidence):** a denser-step walk on the §9.4 shapes finds no count the production walk missed; a miss is recorded as a fixture before any discovery change.
...
12. **Exact adapter identity:** for every Centre mode and governor the exact reconstruction matches the numeric selection (mesh/Box exact; Weight within float-centroid noise); square 25 at pitch 24 returns one exact verdict in fixed and rung paths; Weight squircle 72 returns the identical exact refusal in both; restoring approximate seat admission makes seat and Wrap disagree and fails.
