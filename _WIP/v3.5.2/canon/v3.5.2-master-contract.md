...
Nothing in this section precedes or blocks T1-T4 code construction, with two exceptions that are mandatory B1 gates: fixture 12 (seat/Wrap one geometry) and fixture 17 (holed scaling). Use the rest after the engine and tab code run, or earlier only when they directly help diagnose a live build failure.
...
2. **Wrap:** flap 0 rungs carry a spot-edge `ContactWitness` whose air rounds to 0 mm; a 0.6 mm air reads as flap 1; adapter source variation cannot widen the gate.
3. **Scaling:** across B1-B4, counts are strictly increasing; each rung is the earliest even size accepted for that count; a count owned by a lower band never repeats; a lawful lower count survives a higher count's refusal; the square publishes 1 at 24, and at 72 both the vertical pair (count 2, gap placement) and the 2×2 (count 4) are lawful and both publish, then 8 at 120 and 12 at 168; the squircle publishes 8 in B4 (its B3 seat-only appearance is not ownership); the diamond publishes 1 at 34 (air 0.06 mm → flap 0).
...
10. **Coverage:** every even size in every band is evaluated; no fixture needed — coverage is by construction.
...
12. **Seat/Wrap one geometry (mandatory B1 gate):** for every Centre mode and governor, a placement that seats is never refused by Wrap for sub-ruler air and vice versa; squircle 72 is lawful at flap 0 in every mode (its 1e-14 asymmetry rounds to 0); square 24 at pitch 24 is lawful at flap 0; a mutation that judges seat and Wrap on different geometry fails.
...
17. **Holed scaling (mandatory B1 gate):** one real holed supplied cutout walks B1–B4; a disc overlapping the hole is refused as an illegal seat; a hole segment can be the binding contact witness; Free at an even size and the rung at that size return identical verdicts and witnesses.
