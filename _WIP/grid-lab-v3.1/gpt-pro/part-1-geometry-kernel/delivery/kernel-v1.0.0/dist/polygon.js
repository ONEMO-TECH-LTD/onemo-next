...
export function segmentsIntersectInclusive(a, b, c, d) {
...
export function preparePolygon(input) {
    ...
    if (raw.length < 3) {
        throw new KernelInputError("POLYGON_TOO_FEW_VERTICES", "polygon.vertices", "a simple polygon requires at least three vertices");
    }
    if (pointEquals(raw[0], raw[raw.length - 1])) {
        throw new KernelInputError("REPEATED_CLOSING_VERTEX", `polygon.vertices[${raw.length - 1}]`, "the closing vertex must not repeat the first vertex");
    }
    const seen = new Map();
    for (let index = 0; index < raw.length; index += 1) {
        ...
        if (previous !== undefined) {
            throw new KernelInputError("DUPLICATE_VERTEX", `polygon.vertices[${index}]`, `duplicates polygon.vertices[${previous}]`);
        }
        seen.set(key, index);
    }
    for (let index = 0; index < raw.length; index += 1) {
        ...
        if (pointEquals(raw[index], raw[next])) {
            throw new KernelInputError("ZERO_LENGTH_EDGE", `polygon.vertices[${index}]`, "consecutive vertices form a zero-length edge");
        }
    }
    let areaTwice = 0n;
    for (let index = 0; index < raw.length; index += 1) {
    ...
    if (areaTwice === 0n) {
        throw new KernelInputError("POLYGON_ZERO_AREA", "polygon.vertices", "signed area is zero");
    }
...
function validateAdjacentEdges(vertices) {
    ...
    for (let sharedIndex = 0; sharedIndex < count; sharedIndex += 1) {
        ...
        if (cross(towardPrevious, towardNext) === 0n && dot(towardPrevious, towardNext) > 0n) {
            throw new KernelInputError("POLYGON_NOT_SIMPLE", `polygon.vertices[${sharedIndex}]`, "adjacent edges overlap by backtracking from their shared vertex");
        }
    }
}
function validateNonAdjacentEdges(vertices) {
    const count = vertices.length;
    for (let first = 0; first < count; first += 1) {
        ...
        for (let second = first + 1; second < count; second += 1) {
            ...
            if (segmentsIntersectInclusive(a, b, c, d)) {
                throw new KernelInputError("POLYGON_NOT_SIMPLE", `polygon.edges[${first}],polygon.edges[${second}]`, "non-adjacent polygon edges intersect or touch");
            }
        }
    }
}
function lexicographicallySmallestIndex(vertices) {
    let best = 0;
