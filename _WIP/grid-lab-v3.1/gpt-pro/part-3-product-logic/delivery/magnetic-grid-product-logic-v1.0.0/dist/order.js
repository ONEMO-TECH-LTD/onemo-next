...
export function buildOrdering(candidates, rules) {
    ...
    for (const group of groups) {
        for (let leftIndex = 0; leftIndex < group.candidates.length; leftIndex += 1) {
            for (let rightIndex = leftIndex + 1; rightIndex < group.candidates.length; rightIndex += 1) {
                const left = group.candidates[leftIndex];
                const right = group.candidates[rightIndex];
                if (compareCandidates(left, right, rules).relation !== "tie") {
                    throw new NonTierableOrderingError(group.candidates.map((candidate) => candidate.ref), "pairwise unresolved comparisons are not transitive, so no honest disjoint tier can contain the group");
                }
            }
        }
    }
    ...
    for (let leftGroupIndex = 0; leftGroupIndex < groups.length; leftGroupIndex += 1) {
        for (let rightGroupIndex = leftGroupIndex + 1; rightGroupIndex < groups.length; rightGroupIndex += 1) {
            ...
            const representativeComparison = compareCandidates(leftGroup.representative, rightGroup.representative, rules);
            if (representativeComparison.relation === "tie") {
                throw new NonTierableOrderingError([leftGroup.representative.ref, rightGroup.representative.ref], "distinct tie components remain unresolved");
            }
            for (const left of leftGroup.candidates) {
                for (const right of rightGroup.candidates) {
                    ...
                    if (comparison.relation !== representativeComparison.relation) {
                        throw new NonTierableOrderingError([left.ref, right.ref], "members of proposed tiers do not have one consistent cross-tier order");
                    }
                }
            }
    ...
    for (let higherIndex = 0; higherIndex < groups.length; higherIndex += 1) {
        for (let lowerIndex = higherIndex + 1; lowerIndex < groups.length; lowerIndex += 1) {
            ...
            if (compareCandidates(higher, lower, rules).relation !== "left-higher") {
                throw new NonTierableOrderingError([higher.ref, lower.ref], "supplied rule inputs form a cycle or a non-transitive comparison and cannot be represented as ranked tiers without invention");
            }
        }
    }
