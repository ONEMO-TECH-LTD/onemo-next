...
test("small completeness golden equals the hand-enumerated set exactly", () => {
  ...
  // two held positions: one single each, plus the run and the 1x2 window over both
  assert.equal(output.candidates.length, 4);
  assert.deepEqual(
    output.candidates.map((candidate) => candidate.family).sort(),
    ["full-window", "run", "single", "single"],
  );
});
...
test("the two exposed formal readings are supplied by grammar data, not chosen by the enumerator", () => {
  ...
  // a lone held position is always a `single`; the 1x1 window is an independent alias
  assert.deepEqual(includeOne.candidates.map((candidate) => candidate.family).sort(), ["full-window", "single"]);
  assert.equal(excludeOne.candidates.length, 1);
  assert.equal(excludeOne.candidates[0].family, "single");
...
test("duplicate size values remain distinct source occurrences while arrangement identity stays structural", () => {
  ...
  const output = enumerateCandidates({ measurement, grammar: allFamiliesGrammar });
  // two singles, one run and one 1x2 window per size occurrence, twice over
  assert.equal(output.candidates.length, 8);
