...
- **CI sequence at `fe7787bb`:** `npm run lint` → **5 inherited errors, none in the Law slice**: `grid-origin/page.tsx` (legacy comparator page: set-state-in-effect 41:48; refs-in-render 602:3, 643:24), `grid-engine/page.tsx` 280:5 (set-state-in-effect), `lib/grid-engine/__tests__/separation.test.ts` 447:7 (assign to `module`). `npx tsc --noEmit` clean · `npm test -- --run` 69 files / 666 tests · `npm run build` exit 0. The lint gate is red for reasons outside v3.5.3; those four legacy files need their own bounded follow-up (two are frozen comparator donors — fixing them would break the byte-identical donor guard unless the freeze is re-ruled).
- **M4 live proof (`M4-live-persist-drag.png`):** dials persisted across reload (pad 14, flap 3), drag moved the lattice pattern x 2 → 9.87 and committed, double-click reset to 2, cursor back to `grab`, 0 console errors / 0 warnings.
