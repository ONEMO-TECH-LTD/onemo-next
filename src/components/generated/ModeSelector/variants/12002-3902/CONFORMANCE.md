# Conformance — 12002:3902 @ file v2381318852839647833

| Check | Result |
|---|---|
| Elements (ID map) | 4 |
| Token coverage (styleable) | **100%** (10 var-decls / 10 styleable) |
| Unresolved `var(--…)` | **0** ✅ |
...
| TOKEN VALUE PARITY | **3** ⚠️ DS drift (below) |
...
- .badge · `width` = `--sem-dim-fluid-big-l` → resolves **55.37px** but Figma shows **56px** (css:11) — fix the token build or the Figma variable, not the converter
- .badge2 · `width` = `--sem-dim-fluid-big-l` → resolves **55.37px** but Figma shows **56px** (css:17) — fix the token build or the Figma variable, not the converter
- .badge3 · `width` = `--sem-dim-fluid-big-l` → resolves **55.37px** but Figma shows **56px** (css:24) — fix the token build or the Figma variable, not the converter
