# Conformance — 6108:53400 @ file v2381318852839647833

| Check | Result |
|---|---|
| Elements (ID map) | 2 |
| Token coverage (styleable) | **83%** (5 var-decls / 6 styleable) |
| Unresolved `var(--…)` | **0** ✅ |
...
- .stateDefault · `width` = `--sem-dim-fluid-big-m` → resolves **47.37px** but Figma shows **48px** (css:5) — fix the token build or the Figma variable, not the converter
- .stateDefault · `height` = `--sem-dim-fluid-big-m` → resolves **47.37px** but Figma shows **48px** (css:6) — fix the token build or the Figma variable, not the converter
