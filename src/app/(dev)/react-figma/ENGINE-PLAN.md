...
| Concurrent JSX literal edits to the same declaration (set-jsx-style) — **DECIDED semantic (s58-lead E2 closure)** | **Last-writer-wins.** The client can't reconstruct a JSX literal's raw source text from a computed value, so `expectRaw` is not sent for the style op; safety is instead guaranteed by (a) F1 literal-only refusal — a dynamic binding is never overwritten — and (b) single-buffer atomic read+splice (no TOCTOU). Two rapid literal edits to the same property therefore resolve last-writer-wins, not 409. `set-jsx-text` DOES send `expectRaw` (client has the before-text) and 409s on conflict. |
