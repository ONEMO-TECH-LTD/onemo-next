...
import { prepareSdfBlend } from '@/lib/outline-core/sdf'
...
export default function OutlineEditor({ open, imageUrl, onClose }: OutlineEditorProps) {
  ...
  const [activeAdjust, setActiveAdjust] = useState<'hug' | 'round' | 'smooth' | 'scale' | 'blend' | 'shape' | null>(null) // which adjustment slider/sheet is revealed (mobile: one at a time)
  ...
  const rotateRef = useRef<{ cx: number; cy: number; start: number } | null>(null) // desktop handle drag
  const gestureRef = useRef<{ cx: number; cy: number; start: number } | null>(null) // two-finger twist
  ...
  useEffect(() => {
    ...
    // Blend morphs square ↔ the ORIGINAL BEN silhouette (from spec, y-flipped to editor space) — NOT the
    // current doc, so Hug still returns to the subject even after a full-square bake/save was approved.
    const k = spec?.mmPerPx || 1
    const silhouette: Vec2Px[] = spec
      ? spec.geometryMM.outer.pts.map(([x, y]) => [x / k, H - y / k] as Vec2Px)
      : (d.rings.find((r) => r.role === 'outer')?.nodes ?? []).map((n) => n.p)
    blendSrc.current = { square: [[0, 0], [W, 0], [W, H], [0, H]], silhouette, domain: { minX: 0, minY: 0, width: W, height: H }, image: d.image }
    setBlend(100)
    setRadius(d.style.globalOutlineCornerRadiusPx)
    setSmoothing(Math.round(d.style.smoothing * 100))
    setScale(100)
  ...
  const onSurfacePointerDown = useCallback(
    (e: React.PointerEvent) => {
      ...
      // a second surface finger → two-finger twist (cancel any pending move)
      if (pointersRef.current.size === 2 && !drag) {
        ...
        gestureRef.current = { cx, cy, start: Math.atan2(pts[1][1] - pts[0][1], pts[1][0] - pts[0][0]) }
        return
      }
    ...
    [drawPts, toViewBox, drag, resolved, preview],
  )
  ...
  const onPointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      // two-finger twist → live rotate
      if (gestureRef.current && pointersRef.current.size >= 2) {
        const pts = [...pointersRef.current.values()]
        const { cx, cy, start } = gestureRef.current
        const live = { deg: ((Math.atan2(pts[1][1] - pts[0][1], pts[1][0] - pts[0][0]) - start) * 180) / Math.PI, cx, cy }
        rotateLiveRef.current = live; setRotateLive(live)
        return
      }
  ...
  const onPointerUp = useCallback((e: React.PointerEvent) => {
    ...
    if (gestureRef.current) { if (pointersRef.current.size < 2) { gestureRef.current = null; commitRotate() } return }
    if (moveRef.current) {
  ...
  }, [drag, drawPts, commit, doc.image, commitRotate])
  ...
  // to the image), seeded into our node model so Round/Smooth/Scale/Hug/drag all apply (radius 0 — shapes
  // are exact; the user rounds via Round). Discrete params (sides/points) regenerate immediately; the
  ...
  const pickShape = useCallback((kind: ShapeKind) => {
    ...
    setRadius(0); setSmoothing(0); setScale(100); setSelectedNode(null); setAllSelected(false); setApproved(null)
      ...
      <div className={styles.status}>
        {preview
          ...
          : hasIssues
            ...
            : allSelected
              ? 'All corners selected — round, scale or twist them together'
              : selectedNode
                ? 'Adjust this corner, or drag it'
                : 'Tap inside to select all · drag inside to move · drag points · double-tap to add/remove'}
      </div>
          ...
          {activeAdjust === 'blend' && (
            <>
              ...
              <input
                ...
                value={blendBlur}
                disabled={!blendOn}
                onChange={(e) => setBlendBlur(Number(e.target.value))}
                onPointerUp={() => writeBlend(blendOn, blendBlur)}
                aria-label="Magic blend intensity"
              />
            </>
          )}
        </div>
            ...
            <div className={styles.shapeControls}>
              ...
              {shapeKind === 'star' && (
                <>
                  ...
                  <div className={styles.shapeRow}>
                    <span className={styles.shapeName}>Spikiness</span>
                    <input className={styles.slider} type="range" min={5} max={95} value={shapeSpikiness} onChange={(e) => previewSpikiness(Number(e.target.value))} onPointerUp={commitShape} aria-label="Spikiness" />
                  </div>
                </>
              )}
