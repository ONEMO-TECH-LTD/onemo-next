...
| Blend (magic blend) toggle | `On / Off` | Off → `bgBlur=0` (sharp full photo). On → intensity below. Edit-mode only; affects the 3D front. |
| Blend intensity slider | `0 … 100` → `bgBlur 0..1` | 50 ≈ build default. `ShapedModel` re-composes the front (no re-segment). 3D-only control. |
