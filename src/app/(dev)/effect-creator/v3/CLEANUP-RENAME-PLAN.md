...
The live golden the Creator loads is `data/scenes/golden.onemo` (via `/api/dev/scenes/golden`); `data/scenes/` also holds `default.json` + `golden-v1/v2`; the draco effect templates are in `public/assets/templates/golden-effect-70mm/` — **all app-level, NONE in `studio/`**. `studio/data/scenes/` is EMPTY; `api/dev/scenes/route.ts:10` guards a missing dir (`mkdir … {recursive:true}`) so the kill can't throw. In the studio-kill commit, strip ALL vestigial `STUDIO_SCENES_DIR` refs — `route.ts` (const + `mkdir` + `readdir` + binary-save) AND `[name]/route.ts` (`studioJsonPath`/`studioOnemoPath`) — so the kill is total (else `route.ts:10` re-creates a phantom `studio/data/scenes`).
