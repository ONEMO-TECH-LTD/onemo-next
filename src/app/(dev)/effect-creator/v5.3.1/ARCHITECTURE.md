...
| File | Role | Key contract |
|---|---|---|
| `OutlineEditor.tsx` (349, was 886) | **Phase 4: a thin CLIENT** (ZERO store reach). Binds `useEditor` `{state, actions}`; owns only the canvas-interaction layer (gestures/points/transforms/view/node-bar) + renders the descriptor sheets. On open: a canvas/UI reset + the landing-sheet decision (`state.hadCommittedOnOpen`). | Dock = **Shape · Magic · Adjust**. The UI knows no tool — tools are descriptors. |
| `editor/useEditor.ts` | **Phase 4: the COMPOSER + editor controller.** Owns the session (open-seed / discard[F12] / reset / spec-reseed), the `EditorCtx` (Layer-2 internal), the F8/F12/F16 folds, the picker actions. Takes runtime `toolEnabled` → returns `state.tools` + canvas scene inputs + artwork actions. Composes `useOutlineEditing`. | UI binds `{state, actions}` only (inv 14/16). |
| `editor/descriptors/*` | **Phase 4: the per-tool modules.** `types.ts` (contract: ToolDescriptor + PickerDescriptor + EditorCtx), `index.ts` (`TOOL_REGISTRY`), `shape/*` (radius·curve·detail·offset·simplify·smooth·straighten + shape-pick picker), `image/*` (brightness·contrast·saturate·warmth·preset·tint·vignette·blend·fill). | A tool = a file + a registry line; runtime-disable = `?disable=`. |
| `editor/tool-sheet.tsx` | **Phase 4: the generic sheets.** `ToolSheet` (renders any value tool by `control.kind`: slider/slider-enum/swatches/toggle) + `PickerSheet` (the shape-outlet picker). Replaces the 3 hardcoded sheets. | Renders DATA + calls previewTool/commitTool by id; no descriptor object crosses. |
| `editor/useImageFilters.ts` | **Phase 4: the hero's live image composer.** Builds the live image `EditorCtx` (preview=commit=live `setImageFx`) + drives the SAME image descriptors → `FiltersSurface` is a store-free descriptor-client. | The bundling test spans the hero too. |
| `editor/tool-config.ts` · `image-presets.ts` · `shape-chips.ts` | **Phase 4 data/adapters (React-free):** the runtime `toolEnabled` adapter (URL `?disable=` / config); the pure image-fx data + `fxToPct`/`fxFromPct`; the shape-chip data — so descriptors import data, never UI. | |
| `editor/useOutlineEditing.ts` (115) | The editor's geometry-state engine over `source+adjustments`. `display = resolve(source, preview ?? adjustments)`; 50-deep history. **F8: `commitWithHistory` snapshots pre, commits, pushes history ONLY on `{ok:true}`; verbs return `CommitResult`** (applyAdjustments/seedSource/reBaseline/transformSource). | resolve() is the only truth→display path. |
| `editor/useEditorGestures.ts` (446) | Every pointer/touch handler. Latest-ref pattern. **Phase 4: store-free** — image-pan/zoom (`artwork`) via composer `getArtwork`/`setArtwork`; **F16: `onPointerCancel` + non-committing `cancelStretch` teardown.** | Image mode: single-finger pans the photo, wheel zooms it. |
...
| `editor/EditorCanvas.tsx` (302) | Presentational SVG overlay (memo'd). Renders photo (live fx + blend + pan/zoom), scrim, true-curve path, anchor skeleton, rotate handle, lock chip, crop grips. **F16: surface + grip `onPointerCancel`/`onLostPointerCapture` → no-commit teardown.** | Live preview = Safari-safe DOM filters (CSS `fxFilter` + SVG `feGaussianBlur`). |
...
| `vector-core/path.ts` (157) | `segmentAt`/`segments`/`cubicPoint`/`flattenPath`/`flattenShape`/`toSVGPathD`/`shapeToSVGPathD`/`transformPath`/`transformShape`/`shapeBBox`/`splitCubic`/`signedArea`. (`filletPath`/`filletShape` moved OUT of the public API — test-only in `vector-core/__tests__/fillet-fixtures.ts`.) |
...
| `shape-library/defs.ts` (214) | **17** stock shapes (`VectorShapeKind`) as pure vector data: circle/square/polygon/star/heart/leaf/lens/pill/diamond/plus/bolt + the baked-organic pinched/sparkle/teardrop/squircle/asterisk/bowtie. 10 surface as launch chips (chips.tsx, alongside the daisy/pinwheel/form/blob parametrics); the other 7 (asterisk/bolt/bowtie/leaf/lens/plus/teardrop) are parked defs. |
