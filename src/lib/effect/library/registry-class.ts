...
export interface ClassSpec {
  ...
  /** The one fail-loud lookup; compatibility shim until the package split removes this module. */
...
const REGISTRY_SHAPES: Record<RegistryFamily, { aspect: 'square' | 'frame'; outline: ReadonlyArray<readonly [number, number]> }> = {
  square: { aspect: 'square', outline: [[0, 0], [1, 0], [1, 1], [0, 1]] },
  rectangle: { aspect: 'frame', outline: [[0, 0], [1, 0], [1, 1], [0, 1]] },
  diamond: { aspect: 'square', outline: [[0.5, 0], [1, 0.5], [0.5, 1], [0, 0.5]] },
}

function registrySpec(family: RegistryFamily): LibraryClass {
  const rules = REGISTRY_RULES[family]
  const shape = REGISTRY_SHAPES[family]
  ...
  return {
    ...
    variantOf: (sel, pitchMM) => {
      const frame = frameOf(sel, pitchMM)
      const variant = asVariant(frame)
      if (variant.id !== sel.frameKey) throw new Error('library: unknown variant ' + sel.frameKey)
      return variant
    },
    ...
    open: (current, pitchMM) => {
      ...
      return { ...current, classId: family, geometryId: undefined, frameKey: frameKeyOf(f0), layoutId: pickLayoutName(f0, 'perimeter') }
    },
...
function triangleSpec(): LibraryClass {
  ...
  return {
    ...
    variantOf: (sel, pitchMM) => {
      const geo = geoOf(sel)
      frameOf(sel, pitchMM)
      const variant = asVariant(geo, pitchMM, 0)
      if (variant.id !== sel.geometryId) throw new Error('library: unknown variant ' + sel.geometryId)
      return variant
    },
    ...
    open: (current, pitchMM) => {
      ...
      return {
        ...current, classId: 'triangle', geometryId: t.id, frameKey: frameKeyOf(f),
...
export const CLASS_SPECS: Record<string, LibraryClass> = {
...
export const specOf = (family: LibraryFamily): LibraryClass => {
  const spec = CLASS_SPECS[family]
  if (!spec) throw new Error('library: unknown classId ' + family)
  return spec
}
