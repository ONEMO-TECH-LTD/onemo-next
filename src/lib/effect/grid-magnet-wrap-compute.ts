...
export function wrapBandLadder(
  ...
  // THE CANON GOES IN WHOLE. Dan, 2026-08-30: "build canon route into the sweeper and let sweeper
  // not run full guess work but fine tune it to fit — simple as that."
  //
  // `wrapGroup` is that fine-tuning: it takes the arrangement entire, bisects the size between the
  // floor and the band's top, and computes every lawful origin the whole group could occupy, then
  // seats it on the one nearest the governed anchor. Asking it "is there a size and a position
  // where all of these hold?" is the whole job.
  //
  // WHAT WAS HERE BEFORE, and why it is gone. The canon was first tested node by node against the
  // material at whatever size the walk had reached, and everything that did not land was dropped;
  // only the survivors were wrapped. That inverted the two steps: it threw magnets away BEFORE the
  // search that exists to make them fit, so "8 refused" never meant "8 are impossible", only "8
  // did not happen to land at the sizes and alignments tried". Nobody asked for the thinning, the
  // coverage pass over the canon, or the contest between canon variants; they were inferred from
  // "process it like anything else" and they are not in scope.
  //
  // When the whole canon cannot be held anywhere in the band there is no optimal row. The fewest
  // and most answers below are the comparables that stand in its place — they are already the
  // engine's own, and no canon-shaped substitute is invented.
