...
/** One offerable layout: the frame it came from, the magnets chosen inside it, and what it holds. */
export interface FramedWrap {
  at: WrapAt
  /** The node frame this arrangement lives in — cols x rows lines of the lattice. */
...
/**
 * THE GENERATOR — arrangements come from the FRAME, not from a block of the N nearest seats.
 *
 * A shape's axis pair names the node frame its bounding box can carry (cx by cy lines). The
 * magnets are then chosen from THAT frame's nodes by what they hold, so a tall shape gets a tall
 * arrangement and the head of a duck can carry one while the body carries the rest. The old
 * generator could only ever build a compact rectangle around the group's own middle, which is why
 * every layout came out as a block and why adding a magnet grew the block instead of covering
 * the shape.
 *
 * Frames are tried around the shape's own aspect; for each, the `count` nodes that leave the
 * least material unheld are picked, and that arrangement is solved to its own tightest centred
 * wrap. The winner is the one that holds the most, then the best centred, then the smallest.
 */
export function wrapFramed(
  sized: (mm: number) => Contour, cfg: WrapConfig, count: number, minMM: number, maxMM: number,
): FramedWrap | null {
  const pitch = cfg.pitchMM ?? DEFAULT_PITCH_MM
  const radius = Math.max(PADDING_FLOOR_MM, cfg.paddingMM ?? PADDING_FLOOR_MM)
  ...
  // The shape's own proportions at a working size decide which frames are worth trying: a frame
  // far from the shape's aspect wastes half its nodes on air.
  const probe = sized(Math.min(maxMM, Math.max(minMM, 200)))
  const pb = box(probe.outer.pts)
  const aspect = (pb.maxX - pb.minX) / Math.max(1e-6, pb.maxY - pb.minY)

  const frames: Array<{ cols: number; rows: number; miss: number }> = []
  for (let cols = 1; cols <= 5; cols++) for (let rows = 1; rows <= 5; rows++) {
    if (cols * rows < want) continue
    if (cols * rows > want + 4) continue              // a frame that dwarfs the ask is mostly air
    const fa = cols / rows
    frames.push({ cols, rows, miss: Math.abs(Math.log(fa / aspect)) })
  }
  if (!frames.length) return null
  frames.sort((a, b) => a.miss - b.miss || a.cols * a.rows - b.cols * b.rows)
  const tried = frames.slice(0, 4)                    // the shape's own aspect, nearest first
  const memo = new Map<number, Pt>()                  // one anchor mesh per size, all frames share it

  let won: FramedWrap | null = null
  for (const f of tried) {
    const nodes: Pt[] = []
    for (let ix = 0; ix < f.cols; ix++) for (let iy = 0; iy < f.rows; iy++)
      nodes.push([(ix - (f.cols - 1) / 2) * pitch, (iy - (f.rows - 1) / 2) * pitch])

    // Seat the whole frame first: its tight wrap gives a size at which to judge coverage.
    const fullAt = wrapGroup(sized, cfg, nodes, minMM, maxMM, memo)
    if (!fullAt) continue

    /** Which `want` of the frame's nodes hold the most, judged AT this placement? Greedy on a
     *  point sample of the material — cheap; the answer is measured exactly by Clipper after. */
    const pickAt = (ref: WrapAt): Pt[] => {
      const outer = sized(ref.sizeMM).outer.pts
      const bb2 = box(outer)
      const sample: Pt[] = []
      for (let x = bb2.minX; x <= bb2.maxX; x += 4) for (let y = bb2.minY; y <= bb2.maxY; y += 4)
        if (inside(outer, x, y)) sample.push([x, y])
      const reach = ref.points.map((w) => sample.map((q) => Math.hypot(q[0] - w[0], q[1] - w[1]) <= hold))
      const chosen: number[] = []
      const covered = new Uint8Array(sample.length)
      for (let t = 0; t < want && t < ref.points.length; t++) {
        let pick = -1, gain = -1
        for (let s = 0; s < ref.points.length; s++) {
          if (chosen.includes(s)) continue
          let g = 0
          for (let q = 0; q < sample.length; q++) if (!covered[q] && reach[s][q]) g++
          if (g > gain) { gain = g; pick = s }
        }
        if (pick < 0) break
        chosen.push(pick)
        for (let q = 0; q < sample.length; q++) if (reach[pick][q]) covered[q] = 1
      }
      return chosen.map((s) => nodes[s])
    }

    // The subset wraps SMALLER than the full frame, and coverage at that smaller size is a
    // different picture — choosing once at the frame's size picked spread layouts that then
    // wrapped loose. So choose, solve, and choose again at the size it actually landed on.
    let group = pickAt(fullAt)
    let at = group.length === nodes.length ? fullAt : wrapGroup(sized, cfg, group, minMM, maxMM, memo)
    if (!at) continue
    if (group.length !== nodes.length) {
      const again = pickAt(at)
      if (again.length === group.length && again.some((g, i) => g[0] !== group[i][0] || g[1] !== group[i][1])) {
        const re = wrapGroup(sized, cfg, again, minMM, maxMM, memo)
        if (re) { group = again; at = re }
      }
    }

    const o = sized(at.sizeMM).outer.pts
    const patches = unheldOf(o, at.points, hold)
    const shapeA = Math.abs(areaOfRing(o))
    const unheldPct = shapeA > 0 ? (patches.reduce((s, p) => s + p.areaMM2, 0) / shapeA) * 100 : 0
    const cand: FramedWrap = { at, frame: { cols: f.cols, rows: f.rows }, unheldPct, patches }
    // Holds the most, then sits nearest the centre, then is smallest.
    if (!won
      || cand.unheldPct < won.unheldPct - 0.5
      || (Math.abs(cand.unheldPct - won.unheldPct) <= 0.5
        && (cand.at.centreOffMM < won.at.centreOffMM - 0.5
          || (Math.abs(cand.at.centreOffMM - won.at.centreOffMM) <= 0.5 && cand.at.sizeMM < won.at.sizeMM)))
    ) won = cand
  }
  void radius
  return won
