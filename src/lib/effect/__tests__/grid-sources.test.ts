  ...
  it('starts every curated preset ladder at a multi-anchor S rung, never ONE', () => {
    ...
    for (const preset of PRESETS) {
      ...
      for (const density of DENSITIES) {
        ...
        compared++
        expect(rungs.every((rung) => rung.points >= 2), `${preset}/${density}`).toBe(true)
        expect(rungs.some((rung) => rung.label === 'ONE'), `${preset}/${density}`).toBe(false)
        if (rungs.length) expect(rungs[0].label, `${preset}/${density}`).toBe('S')
      }
    }
