...
describe('8 — recovered phase search is wired through the production Canon solver', () => {
  ...
  it('protection uses physical discs plus only bounded outer-rim spans', () => {
    const rectangle: Contour = { outer: { pts: [[0, 0], [220, 0], [220, 120], [0, 120]] }, holes: [] }
    const heldArea = (contour: Contour, points: Pt[], radii: number[]) => {
      const facts = holdingFactsOf(contour, points, [110, 60], 0, 48, 0, radii)
      return facts.evidence!.materialAreaMM2 - facts.evidence!.areaMM2
    }
    expect(heldArea(rectangle, [[50, 60], [98, 60]], [3, 3])).toBeGreaterThan(300)
    expect(heldArea(rectangle, [[50, 60], [147, 60]], [3, 3])).toBeLessThan(70)
    expect(heldArea(rectangle, [[50, 60], [98, 60]], [4, 4]))
      .toBeGreaterThan(heldArea(rectangle, [[50, 60], [98, 60]], [3, 3]))

    const hole: Contour = {
      outer: rectangle.outer,
      holes: [{ pts: [[68, 45], [80, 45], [80, 75], [68, 75]] }],
    }
    expect(heldArea(hole, [[50, 60], [98, 60]], [3, 3])).toBeLessThan(70)
    const notch: Contour = { outer: { pts: [
      [0, 0], [220, 0], [220, 120], [80, 120], [80, 45], [68, 45], [68, 120], [0, 120],
    ] }, holes: [] }
    expect(heldArea(notch, [[50, 60], [98, 60]], [3, 3])).toBeLessThan(70)
  })

  it('production repair selects a same-count placement before naming Min', () => {
    const sized = (mm: number): Contour => ({ outer: { pts: [
      [0, 0], [mm, 0], [mm, mm * 0.6], [mm * 0.8, mm * 0.6],
      [mm * 0.8, mm], [0, mm],
    ] }, holes: [] })
    const rules = {
      universal: true, balance: false, perimeter: false, extremes: false, ends: false, top: false,
    }
    const offRules = { ...rules, universal: false }
    const canon = canonLayoutForFrame(48, 3, 3)!.nodesMM.map(([x, y]) => [x, y] as Pt)
    const solveOff = (protectionPaddingMM: number) => solveCanonExperiment(sized, {
      pitchMM: 48, paddingMM: 12, perimeterOnly: false, holdingRules: offRules, protectionPaddingMM,
    }, 96, 144, 24, (mm) => [mm / 2, mm / 2], canon, 'repair-off-equivalence')
    const signature = (result: ReturnType<typeof solveOff>) => result.offers.map((offer) => ({
      roles: offer.roles, sizeMM: offer.at.sizeMM, points: offer.at.points,
    }))
    expect(signature(solveOff(0))).toEqual(signature(solveOff(96)))
    const result = solveCanonExperiment(sized, {
      pitchMM: 48, paddingMM: 12, perimeterOnly: false, holdingRules: rules, protectionPaddingMM: 0,
    }, 96, 144, 24, (mm) => [mm / 2, mm / 2], canon,
    'same-count-repair-production')
    expect(result.trace.repairAdmitted).toBeGreaterThan(0)
    expect(result.trace.repairedRoles).toContain('min')
    const min = result.offers.find((offer) => offer.roles.includes('min'))!
    expect(min.at.count).toBe(4)
    expect(result.trace.repairSwaps).toBe(result.trace.repairWraps)
  }, 30_000)
})
