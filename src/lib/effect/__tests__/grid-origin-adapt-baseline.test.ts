...
import { bbox, centroidOf, impliedFlapMM, latticeAt, latticeOver, makeSeatPredicate, maxPressMM, scaleContour, splitPerimeter } from '../grid-origin-compute'
...
const square = (mm: number): Contour => ({ outer: { pts: [[0, 0], [mm, 0], [mm, mm], [0, mm]] }, holes: [] })
...
describe('v3.5 ADAPT donor characterization', () => {
  it('pins numeric bbox/lattice/centroid/scale behavior and the hole-loss defect', () => {
    ...
    const withHole: Contour = { outer: { pts: [[0, 0], [1, 0], [1, 1], [0, 1]] }, holes: [{ pts: [[0.2, 0.2], [0.3, 0.2], [0.2, 0.3]] }] }
    expect(scaleContour(withHole, 96)).toEqual({ outer: { pts: [[0, 0], [96, 0], [96, 96], [0, 96]] }, holes: [] })
  })
  ...
  it('pins band, governor, coverage, and magnet-plan outputs', () => {
  ...
  it('pins first-count/no-repeat and whole-millimetre Auto compatibility', () => {
    ...
    expect([24, 72, 120, 168].flatMap((fromMM) =>
      bandSnapPoints(sized, { positioning: 2, paddingMM: 12, flapMM: 0 }, fromMM, 1)
        .map(({ sizeMM, count }) => [sizeMM, count])))
      .toEqual([[24, 1], [72, 4], [120, 8], [168, 12]])

    const rectangle = (mm: number): Contour => ({ outer: { pts: [[0, 0], [mm, 0], [mm, mm * 0.3], [0, mm * 0.3]] }, holes: [] })
    const auto = autoFlapInBand(rectangle, { positioning: 2, paddingMM: 12 }, 72, 1, 8)
    expect(auto.flapMM).toBe(2)
    expect(auto.fit.ladder).toEqual([{ sizeMM: 100, count: 2 }])
    const capped = autoFlapInBand(rectangle, { positioning: 2, paddingMM: 12 }, 72, 1, 1)
    expect(capped.flapMM).toBe(1)
    expect(capped.fit.ladder).toEqual([])
  })

  it('pins pass-through sizing and bridge view mappings', () => {
    expect(makeSizer(square(1), 0)(72)).toEqual(square(72))
    const grown = makeSizer(square(1), 2)(72)
    const grownBounds = bbox(grown.outer.pts)
    expect(grownBounds.minX).toBeCloseTo(-2, 9)
    expect(grownBounds.minY).toBeCloseTo(-2, 9)
    expect(grownBounds.maxX).toBeCloseTo(74, 9)
    expect(grownBounds.maxY).toBeCloseTo(74, 9)
    ...
    expect(field.filter((spot) => spot.held).map(({ x, y }) => [x, y])).toEqual(grid.anchors.map((anchor) => anchor.p))
  })
})
