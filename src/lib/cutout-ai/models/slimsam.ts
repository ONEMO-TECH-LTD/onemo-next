...
export class SlimSamModel implements SegModel {
  ...
  async load(cfg: SegModelConfig, exec: Exec): Promise<string> {
    ...
    let device = 'wasm'
    if (exec === 'auto') {
      try { this.model = await tx.SamModel.from_pretrained(cfg.id, { dtype: 'fp16', device: 'webgpu' }); device = 'webgpu' }
      catch { this.model = await tx.SamModel.from_pretrained(cfg.id, { dtype: 'q8', device: 'wasm' }) }
    } else {
      try { this.model = await tx.SamModel.from_pretrained(cfg.id, { dtype: 'q8', device: 'wasm' }) }
      catch { this.model = await tx.SamModel.from_pretrained(cfg.id, { dtype: 'fp32', device: 'wasm' }) }
    }
