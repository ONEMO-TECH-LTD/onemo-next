...
export function buildContour(
  ...
  // Product = a SOLID suede cut-out: BEN must not punch interior holes into the piece, even when the
  // silhouette has enclosed gaps (windows, under-arches). Drop detected holes → solid shape (Dan).
  const holes: Ring[] = []
