import { lcm, makeRational, parseInteger, parseRationalPoint, rationalJson, rationalPointFromScaledIntegers, rationalPointJson, } from "./arithmetic.js";
import { KernelInputError } from "./errors.js";
import { buildWorkPolygon, locatePoint, measurePointBoundary, measureSegmentBoundary, squaredDistanceAtLeastRadius, } from "./geometry.js";
import { preparePolygon } from "./polygon.js";
export function measureLattice(input) {
    const polygon = preparePolygon(input.polygon);
    const parameters = parseParameters(input.parameters);
    const sizes = input.sizes.map((sizeInput, sizeIndex) => {
...
export function measureStraightCapsule(input) {
    const polygon = preparePolygon(input.polygon);
    const parameters = parseParameters(input.parameters);
    ...
    const boundary = measureSegmentBoundary(firstPoint, secondPoint, context.polygon, context.workDenominator);
    const fits = firstLocation === "inside" &&
        secondLocation === "inside" &&
        squaredDistanceAtLeastRadius(boundary.workDistanceNumerator, boundary.workDistanceDenominator, context.radiusWork);
    return {
...
function measureOneSize(context) {
    const positions = [];
...
function parseParameters(input) {
    ...
    if (latticePitch <= 0n) {
        throw new KernelInputError("NON_POSITIVE_LATTICE_PITCH", "parameters.lattice.pitch", "lattice pitch must be positive");
    }
    const discDiameter = parseInteger(input.discDiameter, "parameters.discDiameter");
    if (discDiameter <= 0n) {
        throw new KernelInputError("NON_POSITIVE_DISC_DIAMETER", "parameters.discDiameter", "disc diameter must be positive");
    }
    const sourceSize = parseInteger(input.sizeTransform.sourceSize, "parameters.sizeTransform.sourceSize");
    if (sourceSize <= 0n) {
        throw new KernelInputError("NON_POSITIVE_SOURCE_SIZE", "parameters.sizeTransform.sourceSize", "sourceSize must be positive");
    }
    return {
        latticePitch,
        latticeOrigin: parseRationalPoint(input.lattice.origin, "parameters.lattice.origin"),
        fieldExtent: parseFieldExtent(input.lattice.fieldExtent),
        discDiameter,
        sourceSize,
        sourceAnchor: parseRationalPoint(input.sizeTransform.sourceAnchor, "parameters.sizeTransform.sourceAnchor"),
        targetAnchor: parseRationalPoint(input.sizeTransform.targetAnchor, "parameters.sizeTransform.targetAnchor"),
    };
}
function parseFieldExtent(input) {
    const extent = {
    ...
    if (extent.minColumn > extent.maxColumn || extent.minRow > extent.maxRow) {
        throw new KernelInputError("INVALID_FIELD_EXTENT", "parameters.lattice.fieldExtent", "inclusive minimum indices must not exceed maximum indices");
    }
    return extent;
}
function parsePositiveSize(input, path) {
    const size = parseInteger(input, path);
    if (size <= 0n) {
        throw new KernelInputError("NON_POSITIVE_SIZE", path, "every requested size must be positive");
    }
    return size;
}
function buildSizeContext(sourcePolygon, parameters, size) {
    const scale = makeRational(size, parameters.sourceSize);
    const workDenominator = commonWorkDenominator(scale, parameters);
    const vertices = sourcePolygon.vertices.map((vertex) => ({
        x: transformCoordinateToWork(vertex.x, parameters.sourceAnchor.x, parameters.targetAnchor.x, scale, workDenominator),
        y: transformCoordinateToWork(vertex.y, parameters.sourceAnchor.y, parameters.targetAnchor.y, scale, workDenominator),
    }));
    const edges = sourcePolygon.edges.map((edge) => {
...
function commonWorkDenominator(scale, parameters) {
    const factors = [
        2n,
...
function transformCoordinateToWork(sourceCoordinate, sourceAnchor, targetAnchor, scale, workDenominator) {
    const targetFactor = exactQuotient(workDenominator, targetAnchor.denominator, "target-anchor denominator");
    const sourceTermDenominator = scale.denominator * sourceAnchor.denominator;
    const sourceFactor = exactQuotient(workDenominator, sourceTermDenominator, "scaled source-anchor denominator");
    return (targetAnchor.numerator * targetFactor +
        scale.numerator *
            (sourceCoordinate * sourceAnchor.denominator - sourceAnchor.numerator) *
            sourceFactor);
}
function latticePointWork(context, column, row) {
    const denominator = context.workDenominator;
    const origin = context.parameters.latticeOrigin;
    return {
        x: origin.x.numerator *
...
function workPointJson(point, workDenominator) {
    return rationalPointJson(rationalPointFromScaledIntegers(point.x, point.y, workDenominator));
}
function exactQuotient(dividend, divisor, label) {
