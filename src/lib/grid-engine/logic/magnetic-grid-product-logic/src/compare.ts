...
import type { ValidatedCandidate, ValidatedPromotion, ValidatedRules } from "./validate.js";
...
type BaseStage = "gravity" | "tight-wrap" | "regional-support";
...
export function compareCandidates(
  ...
  if (leftPromotion !== undefined && rightPromotion !== undefined) {
    throw new NonTierableOrderingError(
      [left.ref, right.ref],
      "reciprocal escalation directives order each candidate above the other",
    );
  }
...
function baseStages(rules: ValidatedRules): readonly BaseStage[] {
...
function compareBaseStage(
  ...
  if (stage === "tight-wrap") {
    ...
    if (
      !left.judgement.gravity.holdsUpperMaterial ||
      !right.judgement.gravity.holdsUpperMaterial
    ) {
      return { relation: "tie" };
    }
    const comparison = compareOrderedValue(
      left.judgement.tightWrap.value,
      right.judgement.tightWrap.value,
  ...
  const comparison = compareOrderedValue(
    left.judgement.regionalSupport.value,
    right.judgement.regionalSupport.value,
