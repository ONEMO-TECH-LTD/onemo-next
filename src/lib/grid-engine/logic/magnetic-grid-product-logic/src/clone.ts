...
export function cloneExact<T>(value: T, path = "value"): T {
...
function clone(value: unknown, path: string): unknown {
    ...
    for (let index = 0; index < value.length; index += 1) {
      if (!Object.prototype.hasOwnProperty.call(value, index)) {
        throw new ProductLogicInputError(
          "INVALID_EXACT_JSON",
          `${path}[${index}]`,
          "sparse arrays are not exact JSON",
        );
      }
  ...
  if (isPlainRecord(value)) {
    if (Object.getOwnPropertySymbols(value).length !== 0) {
      throw new ProductLogicInputError(
        "INVALID_EXACT_JSON",
        path,
        "symbol-keyed members are not exact JSON",
      );
    }
    ...
    for (const key of Object.keys(value)) {
      ...
      if (member === undefined) {
        throw new ProductLogicInputError(
          "INVALID_EXACT_JSON",
          `${path}.${key}`,
          "undefined is not allowed in exact documents",
        );
      }
  ...
  throw new ProductLogicInputError(
    "INVALID_EXACT_JSON",
    path,
    "documents may contain only exact JSON values; JavaScript number and BigInt are forbidden",
  );
}
