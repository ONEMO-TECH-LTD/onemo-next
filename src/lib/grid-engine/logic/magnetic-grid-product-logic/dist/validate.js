...
import { assertAllowedKeys, assertExactKeys, isRecord, nonemptyString, parseDecimalInteger, parseNonnegativeDecimalInteger, validateExactJson, validateOrderedComparator, validateOrderedValue, validateRational, } from "./exact.js";
import { candidatePointer, parseCandidatePointer, parsePositionPointer, parseSizePointer, } from "./pointer.js";
...
const REGIONAL_PRECEDENCE_SET = new Set([
    "report-only",
    "before-gravity",
...
export function validateInput(input) {
    ...
    assertExactKeys(root, ["candidateDocument", "judgements", "measurementDocument", "rules"], "input");
    ...
    const judgementsByRef = validateJudgements(root.judgements, candidateBases, rules.tightWrap.comparator, rules.regionalSupport.comparator);
    const statusByRef = validateStatusAssignments(rules.statusPolicy, candidateBases);
    const candidates = candidateBases.map((base) => {
        ...
        const status = statusByRef.get(base.ref) ?? { kind: "not-supplied" };
        return {
            ...base,
            bandId,
            bandOrderIndex,
...
function validateCandidateDocument(value) {
    const cloned = cloneExact(value, "input.candidateDocument");
    if (!isRecord(cloned)) {
        throw new ProductLogicInputError("INVALID_INPUT", "input.candidateDocument", "expected an object");
    }
    if (cloned.schema !== "magnetic-grid-candidate-enumerator/candidates/v1") {
        throw new ProductLogicInputError("INVALID_SCHEMA", "input.candidateDocument.schema", "expected magnetic-grid-candidate-enumerator/candidates/v1");
    }
    if (cloned.sourceMeasurementSchema !== "magnetic-grid-measurement-kernel/lattice/v1") {
        throw new ProductLogicInputError("INVALID_SCHEMA", "input.candidateDocument.sourceMeasurementSchema", "expected magnetic-grid-measurement-kernel/lattice/v1");
    }
    if (!Array.isArray(cloned.candidates)) {
        throw new ProductLogicInputError("INVALID_INPUT", "input.candidateDocument.candidates", "expected an array");
    }
    return cloned;
}
function validateMeasurementDocument(value) {
    const cloned = cloneExact(value, "input.measurementDocument");
    if (!isRecord(cloned)) {
        throw new ProductLogicInputError("INVALID_INPUT", "input.measurementDocument", "expected an object");
    }
    if (cloned.schema !== "magnetic-grid-measurement-kernel/lattice/v1") {
        throw new ProductLogicInputError("INVALID_SCHEMA", "input.measurementDocument.schema", "expected magnetic-grid-measurement-kernel/lattice/v1");
    }
    if (!Array.isArray(cloned.sizes)) {
        throw new ProductLogicInputError("INVALID_INPUT", "input.measurementDocument.sizes", "expected an array");
    }
    cloned.sizes.forEach((size, index) => validateSizeMeasurement(size, `input.measurementDocument.sizes[${index}]`));
    return cloned;
}
function validateSizeMeasurement(value, path) {
    if (!isRecord(value)) {
    ...
    value.positions.forEach((position, index) => validatePositionMeasurement(position, `${path}.positions[${index}]`));
}
function validatePositionMeasurement(value, path) {
    if (!isRecord(value)) {
        throw new ProductLogicInputError("INVALID_INPUT", path, "expected an object");
    }
    ...
    if (value.centerLocation !== "inside" &&
        value.centerLocation !== "outside" &&
        value.centerLocation !== "boundary") {
        throw new ProductLogicInputError("INVALID_INPUT", `${path}.centerLocation`, "expected inside, outside, or boundary");
    }
    if (!isRecord(value.clearance) || value.clearance.kind !== "sqrt-rational") {
        throw new ProductLogicInputError("INVALID_INPUT", `${path}.clearance`, "expected a sqrt-rational object");
    }
    ...
    if (!Array.isArray(value.limitingContacts)) {
        throw new ProductLogicInputError("INVALID_INPUT", `${path}.limitingContacts`, "expected an array");
    }
    value.limitingContacts.forEach((contact, index) => validatePointBoundaryContact(contact, `${path}.limitingContacts[${index}]`));
}
function validatePointBoundaryContact(value, path) {
    if (!isRecord(value)) {
        throw new ProductLogicInputError("INVALID_INPUT", path, "expected an object");
    }
...
function validateBoundaryFeature(value, path) {
    if (!isRecord(value)) {
        throw new ProductLogicInputError("INVALID_INPUT", path, "expected an object");
    }
    if (value.kind === "edge") {
    ...
    throw new ProductLogicInputError("INVALID_INPUT", `${path}.kind`, "expected edge or vertex");
}
...
function validateCandidateSources(document, measurement) {
    ...
    document.candidates.forEach((candidateValue, index) => {
        ...
        if (typeof family !== "string" || !FAMILY_SET.has(family)) {
            throw new ProductLogicInputError("INVALID_INPUT", `${path}.family`, "expected single, run, rectangle-corners, corner-triangle, or full-window");
        }
        ...
        const sizeIndexBig = parseNonnegativeDecimalInteger(candidateValue.size.kernelSizeIndex, `${path}.size.kernelSizeIndex`);
        if (sizeIndexBig > BigInt(Number.MAX_SAFE_INTEGER)) {
            throw new ProductLogicInputError("INVALID_POINTER", `${path}.size.kernelSizeIndex`, "size index exceeds JavaScript safe array indexing range");
        }
        ...
        if (pointerIndex !== sizeIndex) {
            throw new ProductLogicInputError("SOURCE_FACT_MISMATCH", `${path}.size.kernelFactRef`, "pointer does not match kernelSizeIndex");
        }
        const sizeFact = measurement.sizes[sizeIndex];
        if (sizeFact === undefined) {
            throw new ProductLogicInputError("MISSING_SOURCE_FACT", `${path}.size.kernelFactRef`, "referenced kernel size fact is absent");
        }
        parseDecimalInteger(candidateValue.size.value, `${path}.size.value`);
        if (candidateValue.size.value !== sizeFact.size) {
            throw new ProductLogicInputError("SOURCE_FACT_MISMATCH", `${path}.size.value`, "candidate size value is not the referenced kernel size value");
        }
        const occurrenceKey = `${sizeIndex}:${id}`;
        if (seenOccurrences.has(occurrenceKey)) {
            throw new ProductLogicInputError("DUPLICATE_INPUT", path, "candidate id is duplicated within the same kernel size occurrence");
        }
        seenOccurrences.add(occurrenceKey);
        if (!Array.isArray(candidateValue.positions) || candidateValue.positions.length === 0) {
            throw new ProductLogicInputError("INVALID_INPUT", `${path}.positions`, "expected a non-empty candidate position set");
        }
        const supportingKernelFacts = [];
        const seenPositionPointers = new Set();
        candidateValue.positions.forEach((positionValue, positionIndex) => {
            ...
            if (seenPositionPointers.has(sourcePointer)) {
                throw new ProductLogicInputError("DUPLICATE_INPUT", `${positionPath}.kernelFactRef`, "candidate position set contains the same kernel fact more than once");
            }
            ...
            if (parsedPointer.sizeIndex !== sizeIndex) {
                throw new ProductLogicInputError("SOURCE_FACT_MISMATCH", `${positionPath}.kernelFactRef`, "position fact belongs to a different kernel size occurrence");
            }
            const sourceFact = sizeFact.positions[parsedPointer.positionIndex];
            if (sourceFact === undefined) {
                throw new ProductLogicInputError("MISSING_SOURCE_FACT", `${positionPath}.kernelFactRef`, "referenced kernel position fact is absent");
            }
            if (positionValue.column !== sourceFact.column ||
                positionValue.row !== sourceFact.row ||
                serializeCanonical(positionValue.center) !== serializeCanonical(sourceFact.center)) {
                throw new ProductLogicInputError("SOURCE_FACT_MISMATCH", positionPath, "candidate index or centre is not an exact copy of its referenced kernel fact");
            }
            if (!sourceFact.fits) {
                throw new ProductLogicInputError("UNHELD_CANDIDATE_POSITION", `${positionPath}.kernelFactRef`, "candidate references a kernel position whose fits fact is false");
            }
...
function validateRules(value, measurement, candidates) {
    if (!isRecord(value)) {
        throw new ProductLogicInputError("INVALID_INPUT", "input.rules", "expected an object");
    }
    assertAllowedKeys(value, ["bands", "gravity", "regionalSupport", "schema", "tightWrap"], ["escalation", "statusPolicy"], "input.rules");
    if (value.schema !== "magnetic-grid-product-logic/rules/v1") {
        throw new ProductLogicInputError("INVALID_SCHEMA", "input.rules.schema", "expected magnetic-grid-product-logic/rules/v1");
    }
    ...
    const regionalSupport = validateRegionalSupportRule(value.regionalSupport, "input.rules.regionalSupport");
    const bands = validateBands(value.bands, measurement.sizes.length, "input.rules.bands");
    const bandOrderIndex = new Map();
    bands.order.forEach((band, index) => bandOrderIndex.set(band, index));
    const sizeBandByIndex = new Map();
    bands.sizeAssignments.forEach((assignment) => sizeBandByIndex.set(Number(BigInt(assignment.kernelSizeIndex)), assignment.band));
    const candidateRefs = new Set(candidates.map((candidate) => candidate.ref));
    const candidateBandByRef = new Map();
    ...
    const escalationResult = validateEscalation(value.escalation, bands, bandOrderIndex, candidateRefs, candidateBandByRef, "input.rules.escalation");
    const statusPolicy = validateStatusPolicyDefinition(value.statusPolicy, "input.rules.statusPolicy");
    return {
        gravity,
        tightWrap,
        regionalSupport,
...
function validateGravityRule(value, path) {
    if (!isRecord(value)) {
        throw new ProductLogicInputError("INVALID_INPUT", path, "expected an object");
    }
...
function validateTightWrapRule(value, path) {
    if (!isRecord(value)) {
        throw new ProductLogicInputError("INVALID_INPUT", path, "expected an object");
    }
...
function validateRegionalSupportRule(value, path) {
    if (!isRecord(value)) {
        throw new ProductLogicInputError("INVALID_INPUT", path, "expected an object");
    }
    assertExactKeys(value, ["comparator", "definition", "definitionId", "precedence"], path);
    if (typeof value.precedence !== "string" ||
        !REGIONAL_PRECEDENCE_SET.has(value.precedence)) {
        throw new ProductLogicInputError("INVALID_RULE_VALUE", `${path}.precedence`, "expected report-only, before-gravity, between-gravity-and-tight-wrap, or after-tight-wrap");
    }
    return {
        definitionId: nonemptyString(value.definitionId, `${path}.definitionId`),
        definition: validateExactJson(value.definition, `${path}.definition`),
...
function validateBands(value, sizeCount, path) {
    if (!isRecord(value)) {
    ...
    if (sizeCount > 0 && value.order.length === 0) {
        throw new ProductLogicInputError("INVALID_BAND_POLICY", `${path}.order`, "at least one band is required when the measurement document has sizes");
    }
    const order = [];
    ...
    value.order.forEach((item, index) => {
        ...
        if (bandSet.has(band)) {
            throw new ProductLogicInputError("DUPLICATE_INPUT", `${path}.order[${index}]`, `duplicate band ${JSON.stringify(band)}`);
        }
        bandSet.add(band);
        order.push(band);
    });
    if (!Array.isArray(value.sizeAssignments)) {
        throw new ProductLogicInputError("INVALID_BAND_POLICY", `${path}.sizeAssignments`, "expected an array");
    }
    ...
    value.sizeAssignments.forEach((assignmentValue, assignmentIndex) => {
        ...
        const exactIndex = parseNonnegativeDecimalInteger(assignmentValue.kernelSizeIndex, `${assignmentPath}.kernelSizeIndex`);
        if (exactIndex > BigInt(Number.MAX_SAFE_INTEGER)) {
            throw new ProductLogicInputError("INVALID_BAND_POLICY", `${assignmentPath}.kernelSizeIndex`, "index exceeds JavaScript safe array indexing range");
        }
        const index = Number(exactIndex);
        if (index >= sizeCount) {
            throw new ProductLogicInputError("INVALID_BAND_POLICY", `${assignmentPath}.kernelSizeIndex`, "referenced kernel size occurrence is absent");
        }
        if (assignmentByIndex.has(index)) {
            throw new ProductLogicInputError("DUPLICATE_INPUT", assignmentPath, "kernel size occurrence is assigned more than once");
        }
        const band = nonemptyString(assignmentValue.band, `${assignmentPath}.band`);
        if (!bandSet.has(band)) {
            throw new ProductLogicInputError("INVALID_BAND_POLICY", `${assignmentPath}.band`, "assigned band is absent from bands.order");
        }
        assignmentByIndex.set(index, { kernelSizeIndex: exactIndex.toString(), band });
    });
    for (let index = 0; index < sizeCount; index += 1) {
        if (!assignmentByIndex.has(index)) {
            throw new ProductLogicInputError("MISSING_INPUT", `${path}.sizeAssignments`, `kernel size occurrence ${index} has no band assignment`);
        }
    }
...
function validateEscalation(value, bands, bandOrderIndex, candidateRefs, candidateBandByRef, path) {
    if (value === undefined) {
        return {
            policy: undefined,
            assessments: new Map(),
    ...
    if (!Array.isArray(value.bandAssessments)) {
        throw new ProductLogicInputError("INVALID_ESCALATION", `${path}.bandAssessments`, "expected an array");
    }
    ...
    value.bandAssessments.forEach((assessmentValue, index) => {
        ...
        assertExactKeys(assessmentValue, ["band", "supportInsufficient", "triggerDefinitionId", "triggerInput"], assessmentPath);
        const band = nonemptyString(assessmentValue.band, `${assessmentPath}.band`);
        const orderIndex = bandOrderIndex.get(band);
        if (orderIndex === undefined) {
            throw new ProductLogicInputError("INVALID_ESCALATION", `${assessmentPath}.band`, "band is absent from bands.order");
        }
        if (assessments.has(band)) {
            throw new ProductLogicInputError("DUPLICATE_INPUT", assessmentPath, `band ${JSON.stringify(band)} is assessed more than once`);
        }
        if (typeof assessmentValue.supportInsufficient !== "boolean") {
            throw new ProductLogicInputError("INVALID_ESCALATION", `${assessmentPath}.supportInsufficient`, "expected a boolean caller ruling");
        }
        const assessment = {
            band,
            supportInsufficient: assessmentValue.supportInsufficient,
            triggerDefinitionId: nonemptyString(assessmentValue.triggerDefinitionId, `${assessmentPath}.triggerDefinitionId`),
    ...
    for (const band of bands.order) {
        if (!assessments.has(band)) {
            throw new ProductLogicInputError("MISSING_INPUT", `${path}.bandAssessments`, `band ${JSON.stringify(band)} has no explicit support-insufficiency ruling`);
        }
    }
    if (!Array.isArray(value.promotions)) {
        throw new ProductLogicInputError("INVALID_ESCALATION", `${path}.promotions`, "expected an array");
    }
    const byTargetAndSource = new Map();
    const mutableByTarget = new Map();
    ...
    value.promotions.forEach((promotionValue, index) => {
        ...
        assertExactKeys(promotionValue, [
            "sourceBand",
        ...
        if (sourceIndex === undefined || targetIndex === undefined) {
            throw new ProductLogicInputError("INVALID_ESCALATION", promotionPath, "sourceBand and targetBand must both appear in bands.order");
        }
        if (targetIndex !== sourceIndex + 1) {
            throw new ProductLogicInputError("INVALID_ESCALATION", `${promotionPath}.targetBand`, "targetBand must be the immediate next band after sourceBand");
        }
        const assessment = assessments.get(sourceBand);
        if (assessment === undefined || !assessment.supportInsufficient) {
            throw new ProductLogicInputError("INVALID_ESCALATION", promotionPath, "a promotion requires an explicit insufficient=true ruling for its source band");
        }
        const targetCandidateRef = nonemptyString(promotionValue.targetCandidateRef, `${promotionPath}.targetCandidateRef`);
        const targetIndexValue = parseCandidatePointer(targetCandidateRef, `${promotionPath}.targetCandidateRef`);
        if (!candidateRefs.has(targetCandidateRef)) {
            throw new ProductLogicInputError("UNKNOWN_CANDIDATE", `${promotionPath}.targetCandidateRef`, `candidate index ${targetIndexValue} is absent`);
        }
        if (candidateBandByRef.get(targetCandidateRef) !== targetBand) {
            throw new ProductLogicInputError("INVALID_ESCALATION", `${promotionPath}.targetCandidateRef`, "target candidate does not belong to targetBand");
        }
        const key = `${targetCandidateRef}\u0000${sourceBand}`;
        if (byTargetAndSource.has(key)) {
            throw new ProductLogicInputError("DUPLICATE_INPUT", promotionPath, "the same target candidate is promoted over the same source band more than once");
        }
        const promotion = {
            sourceBand,
            targetBand,
            targetCandidateRef,
            strengthDefinitionId: nonemptyString(promotionValue.strengthDefinitionId, `${promotionPath}.strengthDefinitionId`),
...
function validateStatusPolicyDefinition(value, path) {
    if (value === undefined) {
        return undefined;
    }
    if (!isRecord(value)) {
    ...
    if (!Array.isArray(value.assignments)) {
        throw new ProductLogicInputError("INVALID_STATUS_POLICY", `${path}.assignments`, "expected an array");
    }
    return {
        ...
        assignments: value.assignments.map((assignment, index) => {
            ...
            if (!isRecord(assignment)) {
                throw new ProductLogicInputError("INVALID_STATUS_POLICY", assignmentPath, "expected an object");
            }
...
function validateJudgements(value, candidates, tightWrapComparator, regionalComparator) {
    ...
    value.forEach((judgementValue, index) => {
        ...
        assertExactKeys(judgementValue, ["candidateRef", "gravity", "regionalSupport", "tightWrap"], path);
        ...
        if (!candidateRefs.has(candidateRef)) {
            throw new ProductLogicInputError("UNKNOWN_CANDIDATE", `${path}.candidateRef`, "candidate pointer is absent from candidateDocument");
        }
        if (output.has(candidateRef)) {
            throw new ProductLogicInputError("DUPLICATE_INPUT", path, "candidate has more than one judgement record");
        }
        ...
        if (typeof judgementValue.gravity.holdsUpperMaterial !== "boolean") {
            throw new ProductLogicInputError("INVALID_RULE_VALUE", `${path}.gravity.holdsUpperMaterial`, "expected a boolean caller judgement");
        }
        ...
        if (!isRecord(judgementValue.regionalSupport)) {
            throw new ProductLogicInputError("INVALID_INPUT", `${path}.regionalSupport`, "expected an object");
        }
        assertExactKeys(judgementValue.regionalSupport, ["basis", "value"], `${path}.regionalSupport`);
        output.set(candidateRef, {
            ...
            tightWrap: {
                value: validateOrderedValue(judgementValue.tightWrap.value, tightWrapComparator, `${path}.tightWrap.value`),
                basis: validateExactJson(judgementValue.tightWrap.basis, `${path}.tightWrap.basis`),
            },
            regionalSupport: {
                value: validateOrderedValue(judgementValue.regionalSupport.value, regionalComparator, `${path}.regionalSupport.value`),
                basis: validateExactJson(judgementValue.regionalSupport.basis, `${path}.regionalSupport.basis`),
            },
        });
    });
    for (const candidate of candidates) {
        if (!output.has(candidate.ref)) {
            throw new ProductLogicInputError("MISSING_INPUT", "input.judgements", `candidate ${candidate.ref} has no gravity, tight-wrap, and regional-support judgements`);
        }
    }
    return output;
}
function validateStatusAssignments(policy, candidates) {
    if (policy === undefined) {
        return new Map();
    }
    const candidateRefs = new Set(candidates.map((candidate) => candidate.ref));
    ...
    policy.assignments.forEach((assignment, index) => {
        ...
        if (output.has(assignment.candidateRef)) {
            throw new ProductLogicInputError("DUPLICATE_INPUT", path, "candidate has more than one status assignment");
        }
    ...
    for (const candidate of candidates) {
        if (!output.has(candidate.ref)) {
            throw new ProductLogicInputError("MISSING_INPUT", "input.rules.statusPolicy.assignments", `candidate ${candidate.ref} has no status assignment under the supplied policy`);
        }
    }
    return output;
}
export function buildCandidateEvaluation(candidate, rules) {
    ...
    const escalationJudgement = rules.escalation === undefined
        ? { kind: "not-supplied" }
        : {
            kind: "supplied",
