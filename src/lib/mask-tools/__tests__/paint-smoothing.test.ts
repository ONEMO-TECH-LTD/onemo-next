...
import { autoTunePaintStroke, paintSmoothingRadius, polishMask, subtractMasks, unionMasks } from '@/lib/mask-tools'
...
describe('Paint inverse erase', () => {
  it('uses subtraction as the exact inverse of Paint union', () => {
    const base = rectangle(24, 24, 2, 2, 22, 22)
    const brush = rectangle(24, 24, 8, 8, 16, 16)

    const added = unionMasks(base, brush)
    const erased = subtractMasks(base, brush)

    for (let index = 0; index < base.data.length; index++) {
      expect(added.data[index]).toBe(base.data[index] || brush.data[index] ? 1 : 0)
      expect(erased.data[index]).toBe(base.data[index] && !brush.data[index] ? 1 : 0)
    }
  })

  it('does not reject internal, splitting, or complete brush subtraction', () => {
    const base = rectangle(24, 24, 2, 2, 22, 22)
    expect(subtractMasks(base, rectangle(24, 24, 8, 8, 16, 16)).data).not.toEqual(base.data)
    expect(subtractMasks(base, rectangle(24, 24, 0, 11, 24, 13)).data).not.toEqual(base.data)
    expect(subtractMasks(base, rectangle(24, 24, 0, 0, 24, 24)).data.every((value) => value === 0)).toBe(true)
  })
})
