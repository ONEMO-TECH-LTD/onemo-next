...
describe('magnetic-grid T1 separation', () => {
  it('is portable and imports only along the declared DAG', () => {
    const allowed: Record<(typeof FILES)[number], readonly string[]> = {
      'spec.ts': [],
      'compute.ts': ['./compute/seat', './compute/centre-evidence', './compute/clearance'],
      'compute/exact-real.ts': [],
      'compute/certified-real.ts': ['./exact-real'],
      ...
      'compute/exact-value.ts': ['../spec', './certified-real', './exact-real', './identity', './polynomial', './region'],
