...
describe('certified angles', () => {
  it('encloses π tightly by Machin', () => {
    ...
    expect(compareExact(pi.lo, rational(31415926535897932384626n, 10000000000000000000000n))).toBeLessThanOrEqual(0)
    expect(compareExact(pi.hi, rational(31415926535897932384627n, 10000000000000000000000n))).toBeGreaterThanOrEqual(0)
  })

  it('encloses atan across the reduction boundary', () => {
    for (const x of [0.25, 1, 3, -0.5, -7]) {
      const i = atanInterval(rational(BigInt(Math.round(x * 4)), 4n), 64n)
      expect(contains(i, Math.atan(x))).toBe(true)
      expect(width(i)).toBeLessThan(1e-14)
    }
    expect(atanInterval(ratFromInt(0), 64n)).toEqual({ lo: ratFromInt(0), hi: ratFromInt(0) })
  })
  ...
  it('angle between directions from exact cross and dot, all quadrants that do not cross the cut', () => {
    ...
    expect(contains(angleBetween(cInt(1), cInt(1), 64n)!, Math.PI / 4)).toBe(true)
    // cross=1, dot=0 → π/2 ; cross=-1, dot=0 → −π/2
    expect(contains(angleBetween(cInt(1), cInt(0), 64n)!, Math.PI / 2)).toBe(true)
    expect(contains(angleBetween(cInt(-1), cInt(0), 64n)!, -Math.PI / 2)).toBe(true)
    // cross=√3, dot=1 → π/3, with an irrational cross
    expect(contains(angleBetween(cSqrt(cInt(3)), cInt(1), 64n)!, Math.PI / 3)).toBe(true)
    // cross=1, dot=-1 → 3π/4 (dot negative, cross positive branch)
    expect(contains(angleBetween(cInt(1), cInt(-1), 64n)!, 3 * Math.PI / 4)).toBe(true)
    // exactly opposite directions sit on the branch cut: refused, not guessed
    expect(angleBetween(cInt(0), cInt(-1), 64n)).toBeNull()
  })
})
