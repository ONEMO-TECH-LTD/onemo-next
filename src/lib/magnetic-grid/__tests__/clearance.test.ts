...
describe('exact clearance kernel', () => {
  it('lifts float coordinates losslessly to one integer unit', () => {
    ...
    expect(c.unit).toBe(4096n)
    expect(toUnits(12, c)).toBe(12n * 4096n)
    expect(compareExact(toMM(12n * 4096n, c), ratFromInt(12))).toBe(0)
  ...
  it('measures squared distance to a segment exactly, interior foot and endpoints', () => {
    ...
    expect(compareExact(segmentDist2(toUnits(3, c), toUnits(4, c), bottom), ratFromInt(16n * u * u))).toBe(0)
    // point (-3,4) past the left endpoint: distance to (0,0) → d² = 25
    expect(compareExact(segmentDist2(toUnits(-3, c), toUnits(4, c), bottom), ratFromInt(25n * u * u))).toBe(0)
    ...
    expect(compareExact(segmentDist2(toUnits(4, ob), toUnits(0, ob), oblique), rational(144n * ob.unit * ob.unit, 25n))).toBe(0)
  })
  ...
  it('finds the nearest boundary and every segment binding a tie', () => {
    ...
    expect(compareExact(centre.d2, ratFromInt(25n * u * u))).toBe(0)
    ...
    expect(compareExact(offCentre.d2, ratFromInt(4n * u * u))).toBe(0)
