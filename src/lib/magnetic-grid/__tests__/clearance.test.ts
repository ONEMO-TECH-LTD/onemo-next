...
describe('exact clearance kernel', () => {
  it('lifts float coordinates losslessly to one integer unit', () => {
    ...
    expect(c.shift).toBe(12) // coarse inputs lift to the 2^12 floor unit; 0.25 = 1·2^-2 fits inside it
    expect(c.unit).toBe(BigInt(4096))
    expect(toUnits(12, c)).toBe(BigInt(12) * BigInt(4096))
    expect(compareExact(toMM(BigInt(12) * BigInt(4096), c), ratFromInt(12))).toBe(0)
    ...
    expect(fine.segments[0].bx).toBe(BigInt('3602879701896397'))
  })

  it('measures squared distance to a segment exactly, interior foot and endpoints', () => {
    const c = exactContour(square(10))
    const bottom = c.segments.find((s) => s.ay === BigInt(0) && s.by === BigInt(0))!
    const u = c.unit
    // point (3,4) above the bottom edge: perpendicular distance 4 → d² = 16
    expect(compareExact(segmentDist2(toUnits(3, c), toUnits(4, c), bottom), ratFromInt(BigInt(16) * u * u))).toBe(0)
    // point (-3,4) past the left endpoint: distance to (0,0) → d² = 25
    expect(compareExact(segmentDist2(toUnits(-3, c), toUnits(4, c), bottom), ratFromInt(BigInt(25) * u * u))).toBe(0)
    // oblique segment (0,0)-(4,3): point (4,0) → cross=12, len2=25 → d² = 144/25
    const ob = exactContour({ outer: { pts: [[0, 0], [4, 3], [0, 3]] }, holes: [] })
    const oblique = ob.segments[1] // (0,0)→(4,3)
    expect(compareExact(segmentDist2(toUnits(4, ob), toUnits(0, ob), oblique), rational(BigInt(144) * ob.unit * ob.unit, BigInt(25)))).toBe(0)
  })

  it('finds the nearest boundary and every segment binding a tie', () => {
    const c = exactContour(square(10))
    const u = c.unit
    const centre = nearestDist2(toUnits(5, c), toUnits(5, c), c)
    expect(compareExact(centre.d2, ratFromInt(BigInt(25) * u * u))).toBe(0)
    expect(centre.binding).toHaveLength(4) // equidistant from all four sides
    const offCentre = nearestDist2(toUnits(2, c), toUnits(5, c), c)
    expect(compareExact(offCentre.d2, ratFromInt(BigInt(4) * u * u))).toBe(0)
  ...
  it('decides containment by exact ray parity, holes included', () => {
    ...
    expect(insideContour(toUnits(2, c), toUnits(2, c), c)).toBe(true)
    expect(insideContour(toUnits(5, c), toUnits(5, c), c)).toBe(false) // inside the hole
    expect(insideContour(toUnits(12, c), toUnits(5, c), c)).toBe(false)
  })
})
