...
export function isolatePrimitiveIntegerRoots(coefficients:readonly string[],lo:Rational,hi:Rational):IsolatedPolynomialRoot[]{const original=primitivePolynomial(coefficients.map(BigInt)).map(value=>rational(value)),out:IsolatedPolynomialRoot[]=[];for(const{factor,multiplicity}of squareFreeFactors(original)){const primitive=rationalPolynomialPrimitive(factor),sequence=sturmSequence(factor),total=rootCount(sequence,lo,hi);const isolate=(left:Rational,right:Rational,count:number)=>{if(!count)return;if(count===1){out.push({primitivePolynomial:primitive,rootIndex:0,isolating:[left,right],multiplicity});return}const middle=divideRational(addRational(left,right),rational(2)),leftCount=rootCount(sequence,left,middle);isolate(left,middle,leftCount);isolate(middle,right,count-leftCount)};isolate(lo,hi,total)}out.sort((a,b)=>compareRational(a.isolating[0],b.isolating[0]));return out.map((root,rootIndex)=>({...root,rootIndex}))}
