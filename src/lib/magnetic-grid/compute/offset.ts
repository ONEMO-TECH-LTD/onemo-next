...
function buildElements(c: ExactContour, r: bigint, nextVertexId: () => number): { elems: Elem[]; feats: ExactSegment[] } {
  ...
  let id = 0
  for (const pts of orientedRings(c)) {
    const n = pts.length
    const edgeSeg: ExactSegment[] = []
    for (let i = 0; i < n; i++) {
      const [ax, ay] = pts[i], [bx, by] = pts[(i + 1) % n]
      const s: ExactSegment = { ax, ay, bx, by, minX: ax < bx ? ax : bx, maxX: ax > bx ? ax : bx, minY: ay < by ? ay : by, maxY: ay > by ? ay : by, ring: 0, edge: i }
...
export function offsetArrangement(c: ExactContour, r: bigint): OffsetArrangement {
  ...
  // coarse certified extents so only elements that can meet are intersected
  const extent = (e: Elem): { minX: number; minY: number; maxX: number; maxY: number } => {
    const rr = Number(r)
    if (e.kind === 'arc') return { minX: Number(e.cx) - rr, maxX: Number(e.cx) + rr, minY: Number(e.cy) - rr, maxY: Number(e.cy) + rr }
    const ax = evaluate(e.a.x, 16n), ay = evaluate(e.a.y, 16n)
    const x0 = Number(ax.lo.n) / Number(ax.lo.d), x1 = Number(ax.hi.n) / Number(ax.hi.d)
    const y0 = Number(ay.lo.n) / Number(ay.lo.d), y1 = Number(ay.hi.n) / Number(ay.hi.d)
    const ex = Number(e.dx), ey = Number(e.dy)
    return { minX: Math.min(x0, x0 + ex) - 1, maxX: Math.max(x1, x1 + ex) + 1, minY: Math.min(y0, y0 + ey) - 1, maxY: Math.max(y1, y1 + ey) + 1 }
  }
  const ext = elems.map(extent)
  const margin = Number(r) // offset lines are cut far outside their nominal span by convex corners
  for (let i = 0; i < elems.length; i++) for (let j = i + 1; j < elems.length; j++) {
    ...
    for (const hit of hits) {
      const v: SharedVertex = { id: vid++, p: hit.p }
      addCut(a, v, hit, 'A'); addCut(b, v, hit, 'B')
    }
  }
