...
/**
 * Forces selected dependency imports to resolve from this editor package.
 * This prevents external prototype files from pulling a second React/R3F stack
 * from the parent worktree node_modules.
 *
 * @param {Record<string, string>} modules - Module name to absolute path.
 * @returns {object} An esbuild plugin.
 */
const pinnedModulesPlugin = modules => ({
    name: 'pinned-modules',
    setup(build) {
        for (const [specifier, resolvedPath] of Object.entries(modules)) {
...
const pagePlugins = [pinnedModulesPlugin(PINNED_VENDOR_MODULES), stubNodeBuiltins, replacePlugin(), polyfillNode()];
