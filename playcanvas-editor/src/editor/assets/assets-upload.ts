...
import { Asset } from '../../editor-api/asset';
...
editor.once('load', () => {
    ...
    editor.method('assets:uploadFile', (args, fn) => {
        ...
        request
        .on('load', (status, data) => {
            if (data && data.id && !editor.call('assets:get', data.id)) {
                editor.call('assets:add', new Asset(data).observer);
            }
    ...
    function uploadFile(currentFolder: { get: (path: string) => unknown } | null, file: File, multipleFiles: boolean) {
        ...
        const uploadToFolder = (folder) => {
            editor.call('assets:uploadFile', {
                ...
                const uploadedAsset = data && data.id ? editor.call('assets:get', data.id) : null;
                if (uploadedAsset) {
                    const uploadedPath = uploadedAsset.get('path') || [];
                    if (uploadedPath.length) {
                        editor.call('assets:panel:currentFolder', editor.call('assets:get', uploadedPath[uploadedPath.length - 1]));
                    } else if (!multipleFiles) {
                        editor.call('assets:panel:currentFolder', null);
                    }

                    if (!multipleFiles) {
                        editor.api.globals.selection.set([uploadedAsset.apiAsset]);
                    }
                }
