...
const MATERIAL_ATTRIBUTES: Attribute[] = [{
    label: 'Color',
    path: 'data.diffuse',
    type: 'rgb'
}, {
...
class RenderComponentInspector extends ComponentInspector {
    ...
    constructor(args: ComponentInspectorArgs) {
        ...
        this._materialPanel = new Panel({
            headerText: 'MATERIAL',
            collapsible: true
        });
        this._materialPanel.hidden = true;
        this.append(this._materialPanel);

        this._materialInspector = new AttributesInspector({
            assets: args.assets,
            entities: args.entities,
            history: args.history,
    ...
    _linkMaterialInspector() {
        ...
        const materialAssets: Observer[] = [];
        const seen = new Set<number>();

        this._entities.forEach((entity) => {
            const assetIds = entity.get('components.render.materialAssets') || [];
            assetIds.forEach((assetId: number | null) => {
                if (assetId === null || assetId === undefined || seen.has(assetId)) {
                    return;
                }
        ...
        this._materialPanel.hidden = false;
        this._materialInspector.link(materialAssets);
    }
    ...
    unlink() {
        this._entityEvents.forEach(event => event.unbind());
